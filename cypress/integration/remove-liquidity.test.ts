describe('Remove Liquidity', () => {
  ...
  it('bnb remove', () => {
    cy.visit('/remove/DNY/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82')
    cy.get('#remove-liquidity-tokena-symbol').should('contain.text', 'DNY')
    cy.get('#remove-liquidity-tokenb-symbol').should('contain.text', 'CAKE')
  })

  it('bnb remove swap order', () => {
    cy.visit('/remove/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82/DNY')
    cy.get('#remove-liquidity-tokena-symbol').should('contain.text', 'CAKE')
    cy.get('#remove-liquidity-tokenb-symbol').should('contain.text', 'DNY')
  })

  it('loads the two correct tokens', () => {
    cy.visit('/remove/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82-0xEfE40EC15b00FeC37047260f54b07cE701Aeb763')
    cy.get('#remove-liquidity-tokena-symbol').should('contain.text', 'CAKE')
    cy.get('#remove-liquidity-tokenb-symbol').should('contain.text', 'DUSD')
  })
